classdef GMEstimIn < EstimIn
    % GMEstimIn:  Gaussian Mixture scalar input estimation function
    ...
    methods
        ...
        function [Uhat, Uvar, NKL] = estim(obj, Rhat, Rvar)
            ...
            for i = 1:L
               ...
               alpha(:,:,i) = (Rhat-obj.theta(:,:,i)).^2./beta(:,:,i);
            ...
            if (nargout >= 3)                            
                NKL = log(sum(obj.omega.*exp(-alpha/2)./sqrt(beta),3)...
        ...
        function x = genRand(obj, nx)            
                ...
                for i = 1:L
                    dummy3(:,i) = (dummy2>=dummy(i) & dummy2<dummy(i+1))...
                        .*(obj.theta(i) + sqrt(obj.phi(i)).*randn(nx,1));
                end
        ...
        function py = plikey(obj,Y,Yvar)
            ...
            for i = 1:L
                lik(:,:,i) = obj.omega(:,:,i).*exp(-1./(2*(obj.phi(:,:,i)+Yvar))...
                    .*(real(Y)-obj.theta(:,:,i)).^2)./sqrt(2*pi*(obj.phi(:,:,i)+Yvar));
            end

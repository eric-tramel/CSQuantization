...
% Version 2.0
%%
function [lambda, theta, phi] = CBG_update(Rhat, Rvar, lambda, theta, phi, EMopt)
...
A_n = (1-lambda)./(lambda).*phi./nu_n.*exp((abs(Rhat-theta).^2./temp-abs(Rhat).^2./Rvar));
A_n = 1./(1+A_n);
A_n(isnan(A_n)) = 0.999;
temp2 = sum(A_n);
...
if EMopt.learn_var
    if strcmp(EMopt.sig_dim,'joint')
        phi = sum(sum(A_n.*(nu_n+abs(gamma_n-theta).^2)))/sum(temp2);
        phi = repmat(phi,[N T]);
    else
        phi = sum(A_n.*(nu_n+abs(gamma_n-theta).^2))./temp2;
        phi = repmat(phi,[N 1]);
    end
end

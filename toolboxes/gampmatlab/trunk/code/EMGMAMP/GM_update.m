...
% Version 2.0
%%
function [lambda, omega, theta, phi] = GM_update(Rhat, Rvar, lambda, omega, theta, phi, EMopt)
...
for i = 1:L
    dummy = Rvar+phi(:,:,i);
    D_l(:,:,i) = lambda.*omega(:,:,i)./sqrt(dummy)...
        .*exp(-abs(theta(:,:,i)-Rhat).^2./(2*dummy));
    gamma(:,:,i) = (Rhat.*phi(:,:,i)+Rvar.*theta(:,:,i))./dummy;
    nu (:,:,i) = (Rvar.*phi(:,:,i))./dummy;
    a_nl(:,:,i) = sqrt(Rvar./(dummy)).*omega(:,:,i)...
    .*exp(((Rhat-theta(:,:,i)).^2./abs(dummy)-abs(Rhat).^2./Rvar)./(-2));
end;
...
if strcmp(EMopt.sig_dim,'joint')
    if ~(T == 0 || T == 1 || N ==0 || N == 1)
        ...
        if EMopt.learn_var
            phi = resize(sum(sum(E_l.*(nu+(gamma-theta).^2)))./N_l,N,T,L);
        end
    ...
    if ~(N == 0 || N == 1)
        ...
        if EMopt.learn_var
            phi = resize(sum(E_l.*(nu+(gamma-theta).^2))./N_l,N,T,L);
        end
        if EMopt.learn_weights
            omega = N_l/N;
            omega = omega./repmat(sum(omega, 3), [1, 1, L]);
